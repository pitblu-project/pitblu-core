...
from pitblu_core.api_client import ApiResponse
from pitblu_core.install_cli import InstallApplication, build_parser
...
def test_dependency_install_defaults_to_no(tmp_path: Path, monkeypatch: Any) -> None:
    ...
    runner = FakeRunner(_dependency_responses(missing="bluez"))
    monkeypatch.setattr(
    ...
    app = InstallApplication(
        ...
        foreground=lambda command: foreground.append(tuple(command)) or 0,
...
def test_fresh_install_delegates_fixed_commands_and_warns_about_one_time_token(
    ...
    monkeypatch.setattr(
        ...
        lambda: type(
            "HealthyClient",
            (),
            {"get": lambda self, path, authenticated=False: ApiResponse(200, {"status": "ok"})},
        )(),
    )
    app = InstallApplication(
        source_root=tmp_path,
        terminal=terminal,
        command_runner=runner,  # type: ignore[arg-type]
        foreground=lambda command: commands.append(tuple(command)) or 0,
        deployment_root=tmp_path / "opt",
        unit_path=tmp_path / "unit",
        effective_uid=lambda: 1000,
    )
